...
{
    public class Synchronization_Quaternion : ISynchronization
    {
        /// <summary>
        ...
        //private Quaternion oldPosition = Quaternion.zero;
        ...
        public Synchronization_Quaternion(CodeInfo codeInfo, EventValueType<PhotonBehaviour.CodeOwner> isOwner, MemberInfo memberInfo, object parent, List<PhotonAttribute> listPhotonAttribute)
        {
            ...
            object oldValue = Quaternion.identity;
            Quaternion oldPosition = Quaternion.identity;
            if (this.get_ValueLocal(out oldValue))
            {
                oldPosition = (Quaternion)oldValue;
            }
        ...
        private void SendStatus()
        {
            ...
            if (this.get_ValueLocal(out oldValue))
            {
                Quaternion oldPosition = (Quaternion)oldValue;
                //Отправляем сообщений
                if ((!this.isMessageNetwork) && (this.onChange_Value != null))
                {
                    this.onChange_Value(this.memberInfo.Name, new object[] { oldPosition.x, oldPosition.y, oldPosition.z, oldPosition.w });
                }
            }
        }
        ...
        private bool OnControl_Delta(ref object oldPosition)
        {
            ...
            if (this.get_ValueLocal(out oldValue))
            {
                if (Quaternion.Angle((Quaternion)oldPosition, (Quaternion)oldValue) >= this.deltaValue)
                {
                    oldPosition = (Quaternion)oldValue;
                    return true;
                }
        ...
        private bool OnControl_Default(ref object oldPosition)
        {
            ...
            if (this.get_ValueLocal(out oldValue))
            {
                if (Quaternion.Angle((Quaternion)oldPosition, (Quaternion)oldValue) >= 0)
                {
                    oldPosition = (Quaternion)oldValue;
                    return true;
                }
        ...
        private bool GET_ValuePhoton_PropertyInfo(out object value)
        {
            ...
            try
            {
                Quaternion vec = (Quaternion)((PropertyInfo)this.memberInfo).GetValue(parent, null);
                value = new object[] { vec.x, vec.y, vec.z, vec.w };
                return true;
            }
            catch { }
            return false;
        ...
        private bool GET_ValuePhoton_FieldInfo(out object value)
        {
            ...
            try
            {
                Quaternion vec = (Quaternion)((FieldInfo)this.memberInfo).GetValue(parent);
                value = new object[] { vec.x, vec.y, vec.z, vec.w };
                return true;
            }
            catch { }
        ...
        void ISynchronization.SET_ValueNetwork(object value)
        {
            ...
            try
            {
                ...
                Quaternion vec = new Quaternion((float)array[0], (float)array[1], (float)array[2], (float)array[3]);
